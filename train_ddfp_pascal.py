    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        if cfg_trainer["eval_on"] and (epoch+1) % 4 == 0:
            ...
            if  epoch >= cfg["trainer"].get("nf_start_epoch", 1):
                # prec_nf = validate_nf(model, nf_model, prior, val_loader, epoch, logger)
...
def validate_nf(
    ...
    for step, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            fts = model(images)['fts']
            
            # NF prediction
            b,c,h,w = fts.size()
            total_n = int(b*h*w)
            fts_reshape = fts.permute(0,2,3,1).contiguous().view(b*h*w, c)
            
            z, _ = nf_model(fts_reshape)
            output_nf = prior.class_logits(z)
            output_nf = output_nf.view(b,h,w,-1).permute(0,3,1,2)            
            
            output = F.interpolate(output_nf, labels.shape[1:], mode="bilinear", align_corners=True)
