...
class city_dset(BaseDataset):
    def __init__(
        self, data_root, data_list, seed=0, n_sup=2975, split="val", img_size=800
    ):
...
def build_city_semi_loader(all_cfg, seed=0):
    cfg_dset = all_cfg["dataset"]
    ...
    # build transform
    dset = city_dset(cfg["data_root"], cfg["data_list"], seed, n_sup, 'sup', cfg['train_crop_size'])
