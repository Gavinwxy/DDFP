...
#TODO
def generate_unsup_data_dual(data_1, data_2, target_1, target_2):
    batch_size, _, im_h, im_w = data_1.shape
    ...
    for i in range(batch_size):
        ...
        new_data.append(
            (
                data_1[i] * mix_mask + data_2[i] * (1 - mix_mask)
            ).unsqueeze(0)
        )
        new_target.append(
            (
                target_1[i] * mix_mask + target_2[i] * (1 - mix_mask)
            ).unsqueeze(0)
        )
